...
export const CreateNoticeObjectModal = ({ visible, onClose, selectedRow, type, handleList }) => {
    ...
    const cards = [
        ...
        { imgSrc: WeChatImg, text: '企业微信应用', value: 'WeChatWork' },
    ...
    useEffect(() => {
        ...
        if (selectedRow) {
            ...
            if (selectedRow.noticeType === 'WeChatWork' && selectedRow.hook) {
                try {
                    const url = new URL(selectedRow.hook)
                    const params = url.searchParams
                    formValues.corpId = params.get('corp_id') || ''
                    formValues.agentId = params.get('agentid') || ''
                    formValues.secret = params.get('corp_secret') || ''
                    formValues.toUser = params.get('touser') || '@all'
                } catch (error) {
                    console.error('Failed to parse WeChatWork hook URL:', error)
                }
            }
    ...
    const handleTestNotice = async () => {
        ...
        if (noticeType === 'WeChatWork') {
            ...
            const toUser = form.getFieldValue('toUser') || '@all'
            if (corpId && agentId && secret) {
                params.hook = `https://qyapi.weixin.qq.com/cgi-bin/message/send?corp_id=${corpId}&corp_secret=${secret}&agentid=${agentId}&touser=${toUser}`
            }
        }
                ...
                {selectedNoticeCard === 4 && (
                    <>
                        <MyFormItem
                            name="corpId"
                            label="企业ID"
                            tooltip="企业微信的企业ID (CorpId)"
                            style={{ marginRight: '10px', width: '100%' }}
                            rules={[{ required: true }]}
                        >
                            <Input placeholder="wwxxxxxxxxxxxxxxx" />
                        </MyFormItem>

                        <MyFormItem
                            name="agentId"
                            label="应用ID"
                            tooltip="企业微信应用的ID (AgentId)"
                            style={{ marginRight: '10px', width: '100%' }}
                            rules={[{ required: true }]}
                        >
                        ...
                        <MyFormItem
                            name="toUser"
                            label="接收者"
                            tooltip="接收者用户ID，多个用|分隔，@all表示所有人"
                            style={{ marginRight: '10px', width: '100%' }}
                            rules={[{ required: true }]}
                        >
                            <Input placeholder="@all 或 user1|user2" />
                        </MyFormItem>
                    </>
                )}
                                    ...
                                    return (
                                        ...
                                        >
                                            <div style={{display: 'flex', gap: 8}}>
                                                <div style={{width: '100%'}}>
                                                    {selectedNoticeCard !== 1 && selectedNoticeCard !== 4 && (
                                                        <Form.Item
                                                            {...restField}
                                                            name={[name, "hook"]}
                                                            label="Hook"
