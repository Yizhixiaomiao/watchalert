...
export const AlertRule = ({ type }) => {
    ...
    const initBasicInfo = (selectedRow) => {
        ...
        form.setFieldsValue({
            ...
            clickhouseConfig: {
                logQL: selectedRow?.clickhouseConfig?.logQL,
                valueField: selectedRow?.clickhouseConfig?.valueField,
                labelFields: selectedRow?.clickhouseConfig?.labelFields,
                annotations: selectedRow?.clickhouseConfig?.annotations,
            },
                ...
                <div>
                        ...
                        <MyFormItemGroup prefix={['clickhouseConfig']}>
                            ...
                            <div className="log-rule-config-container">
                                ...
                                >
                                    <TextArea 
                                        rows={4}
                                        placeholder="请输入 SQL 查询语句，例如：SELECT domain, COUNT(*) as cnt FROM nginx_logs WHERE timestamp >= now() - INTERVAL 1 MINUTE GROUP BY domain"
                                        maxLength={10000}
                                ...
                                <Button
                                    type="primary"
                                    style={{backgroundColor: '#000', borderColor: '#000', width: '120px'}}
                                    onClick={() => {
                                        if (selectedItems.length === 0) {
                                            message.error("请先选择数据源")
                                            return
                                ...
                                <div style={{marginTop: '20px', padding: '16px', backgroundColor: '#fafafa', borderRadius: '4px', border: '1px solid #e8e8e8'}}>
                                    <div style={{marginBottom: '12px'}}>
                                        <Typography.Text strong style={{fontSize: '14px'}}>
                                            高级模式（可选）
                                        </Typography.Text>
                                        <Typography.Text type="secondary" style={{display: 'block', fontSize: '12px', marginTop: '4px'}}>
                                            配置后将从查询结果中提取指定字段值进行告警判断。例如：查询返回 cnt_4xx 和 domain 字段，可选择 cnt_4xx 作为判断条件，domain 用于告警详情。
                                        </Typography.Text>
                                    </div>
                                    ...
                                    <MyFormItem
                                        name="valueField"
                                        label="数值字段（用于告警判断）"
                                        tooltip="从查询结果中选择一个数值字段用于告警条件判断。例如：cnt_4xx、avg_cpu、total_count 等"
                                        style={{marginBottom: '12px'}}
                                    >
                                        <Input 
                                            placeholder="例如：cnt_4xx、avg_cpu、total_count（填写后启用高级模式）" 
                                            allowClear
                                        />
                                    </MyFormItem>
                                    ...
                                    <MyFormItem
                                        name="labelFields"
                                        label="标签字段（用于分组告警）"
                                        tooltip="指定哪些字段作为标签用于生成独立告警。例如：domain、host、region。每个标签组合生成一个独立告警事件。留空则使用所有查询字段"
                                    >
                                        <Select
                                            mode="tags"
                                            style={{width: '100%'}}
                                            placeholder="输入字段名后按 Enter 添加，例如：domain, host（留空则使用所有字段）"
                                            allowClear
                                            tokenSeparators={[',']}
                                        />
                                    </MyFormItem>
                                    ...
                                    <MyFormItem
                                        name="annotations"
                                        label="告警详情（可选）"
                                        tooltip="自定义告警详情模板，可使用查询结果的所有字段。例如：${labels.domain}, ${labels.cnt_4xx} 等"
                                    >
                                        <TextArea 
                                            rows={2}
                                            placeholder="例如：域名 ${labels.domain} 出现 ${labels.cnt_4xx} 次 4xx 错误，请尽快处理！"
                                            maxLength={10000}
                                        />
                                    </MyFormItem>
                                </div>
                    ...
                    {(selectedType === 1 || selectedType === 2 || selectedType === 7 || selectedType === 8 || selectedType === 9) && (
                        <MyFormItem
                            ...
                            rules={[{ required: true, message: '请输入告警条件' }]}
                            style={{ width: '100%' }}
                            tooltip={selectedType === 9 ? "ClickHouse: 如果配置了数值字段，则基于该字段值判断；否则基于日志数量判断" : undefined}
                        >
                            <Input
                                addonBefore={
                                    selectedType === 9 
                                        ? (form.getFieldValue(['clickhouseConfig', 'valueField']) 
                                            ? `当字段 ${form.getFieldValue(['clickhouseConfig', 'valueField']) || '[数值字段]'}` 
                                            : "当日志条数")
                                        : "当日志条数"
                                }
                                addonAfter="时触发告警"
                                placeholder={
                                    selectedType === 9 && form.getFieldValue(['clickhouseConfig', 'valueField'])
                                        ? "例如：> 10（基于数值字段判断）"
                                        : "请输入有效的告警条件，例如：> 2"
                                }
