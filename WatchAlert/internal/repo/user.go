package repo

import (
	"context"
	"fmt"
	"github.com/bytedance/sonic"
	"github.com/zeromicro/go-zero/core/logc"
	"gorm.io/gorm"
	"time"
	"watchAlert/internal/models"
	"watchAlert/pkg/client"
	"watchAlert/pkg/tools"
)

type (
	UserRepo struct {
		entryRepo
	}

	InterUserRepo interface {
		List(query, joinDuty, tenantId string) ([]models.Member, error)
		Get(userId, username, query string) (models.Member, bool, error)
		Create(r models.Member) error
		Update(r models.Member) error
		Delete(userId string) error
		ChangeCache(userId string)
		ChangePass(userId, password string) error
		UpdateStatus(userId, status, reason string) error
		GetStatusHistory(userId string, page, size int) ([]models.UserStatusHistory, error)
		AddActivityLog(log models.UserActivityLog) error
		GetActivityLogs(userId, userName, action, resourceType string, page, size int) ([]models.UserActivityLog, error)
	}
)

func newUserInterface(db *gorm.DB, g InterGormDBCli) InterUserRepo {
	return &UserRepo{
		entryRepo{
			g:  g,
			db: db,
		},
	}
}

func (ur UserRepo) List(query, joinDuty, tenantId string) ([]models.Member, error) {
	var (
		data []models.Member
		db   = ur.db.Model(&models.Member{})
	)

	if query != "" {
		db.Where("user_name LIKE ? OR email Like ? OR phone LIKE ?", "%"+query+"%", "%"+query+"%", "%"+query+"%")
	}
	if joinDuty == "true" {
		db.Where("join_duty = ?", "true")
	}
	if tenantId != "" {
		db.Where("JSON_CONTAINS(tenants, ?)", `"`+tenantId+`"`)
	}
	err := db.Find(&data).Error
	if err != nil {
		return nil, err
	}

	return data, nil
}

func (ur UserRepo) Get(userId, username, query string) (models.Member, bool, error) {
	var (
		data models.Member
		db   = ur.db.Model(models.Member{})
	)

	if userId != "" {
		db.Where("user_id = ?", userId)
	}
	if username != "" {
		db.Where("user_name = ?", username)
	}
	if query != "" {
		db.Where("user_id LIKE ? or user_name LIKE ? or email LIKE ? or phone LIKE ?", "%"+query+"%", "%"+query+"%", "%"+query+"%", "%"+query+"%")
	}

	err := db.First(&data).Error
	if err != nil {
		if err == gorm.ErrRecordNotFound {
			return data, false, fmt.Errorf("用户不存在")
		}
		return data, false, err
	}

	return data, true, nil
}

func (ur UserRepo) Create(r models.Member) error {
	err := ur.g.Create(models.Member{}, r)
	if err != nil {
		return err
	}

	if r.UserId == "admin" {
		r.Tenants = append(r.Tenants, "default")
		err = ur.g.Updates(Updates{
			Table: models.Member{},
			Where: map[string]interface{}{
				"user_id = ?": r.UserId,
			},
			Updates: r,
		})
	}

	return nil
}

func (ur UserRepo) Update(r models.Member) error {
	u := Updates{
		Table: models.Member{},
		Where: map[string]interface{}{
			"user_id = ?": r.UserId,
		},
		Updates: r,
	}

	err := ur.g.Updates(u)
	if err != nil {
		return err
	}

	return nil
}

func (ur UserRepo) Delete(userId string) error {
	userInfo, _, err := ur.User().Get(userId, "", "")
	if err != nil {
		return err
	}

	for _, tid := range userInfo.Tenants {
		err = ur.Tenant().RemoveTenantLinkedUsers(tid, userId)
		if err != nil {
			return err
		}
	}

	d := Delete{
		Table: models.Member{},
		Where: map[string]interface{}{
			"user_id = ?": userId,
		},
	}
	err = ur.g.Delete(d)
	if err != nil {
		return err
	}

	return nil
}

func (ur UserRepo) ChangeCache(userId string) {
	var dbUser models.Member
	ur.db.Model(&models.Member{}).Where("user_id = ?", userId).First(&dbUser)

	var cacheUser models.Member
	result, err := client.Redis.Get("uid-" + userId).Result()
	if err != nil {
		logc.Error(context.Background(), err)
	}
	_ = sonic.Unmarshal([]byte(result), &cacheUser)

	duration, _ := client.Redis.TTL("uid-" + userId).Result()
	client.Redis.Set("uid-"+userId, tools.JsonMarshalToString(dbUser), duration)
}

func (ur UserRepo) ChangePass(userId, password string) error {
	u := Update{
		Table: models.Member{},
		Where: map[string]interface{}{
			"user_id = ?": userId,
		},
		Update: []string{"password", password},
	}

	err := ur.g.Update(u)
	if err != nil {
		return err
	}

	return nil
}

// UpdateStatus 更新用户状态
func (ur UserRepo) UpdateStatus(userId, status, reason string) error {
	u := Update{
		Table: models.Member{},
		Where: map[string]interface{}{
			"user_id = ?": userId,
		},
		Update: []string{"status", status},
	}

	err := ur.g.Update(u)
	if err != nil {
		return err
	}

	return nil
}

// GetStatusHistory 获取用户状态历史
func (ur UserRepo) GetStatusHistory(userId string, page, size int) ([]models.UserStatusHistory, error) {
	var history []models.UserStatusHistory

	db := ur.db.Model(&models.UserStatusHistory{})
	db.Where("user_id = ?", userId).
		Order("created_at DESC").
		Limit(size).
		Offset((page - 1) * size)

	err := db.Find(&history).Error
	if err != nil {
		return nil, err
	}

	return history, nil
}

// AddStatusHistory 添加状态变更历史
func (ur UserRepo) AddStatusHistory(userId, oldStatus, newStatus, reason, operatorId, operatorName string) error {
	history := models.UserStatusHistory{
		Id:           "ush-" + tools.RandId(),
		UserId:       userId,
		OldStatus:    oldStatus,
		NewStatus:    newStatus,
		Reason:       reason,
		OperatorId:   operatorId,
		OperatorName: operatorName,
		CreatedAt:    time.Now().Unix(),
	}

	return ur.g.Create(models.UserStatusHistory{}, history)
}

// AddActivityLog 添加用户活动日志
func (ur UserRepo) AddActivityLog(log models.UserActivityLog) error {
	log.Id = "ual-" + tools.RandId()
	log.CreatedAt = time.Now().Unix()
	return ur.g.Create(models.UserActivityLog{}, log)
}

// GetActivityLogs 获取用户活动日志
func (ur UserRepo) GetActivityLogs(userId, userName, action, resourceType string, page, size int) ([]models.UserActivityLog, error) {
	var logs []models.UserActivityLog

	db := ur.db.Model(&models.UserActivityLog{})

	if userId != "" {
		db.Where("user_id = ?", userId)
	}
	if userName != "" {
		db.Where("user_name LIKE ?", "%"+userName+"%")
	}
	if action != "" {
		db.Where("action = ?", action)
	}
	if resourceType != "" {
		db.Where("resource_type = ?", resourceType)
	}

	db.Order("created_at DESC").
		Limit(size).
		Offset((page - 1) * size)

	err := db.Find(&logs).Error
	if err != nil {
		return nil, err
	}

	return logs, nil
}
