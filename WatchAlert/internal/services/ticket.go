...
func (s ticketService) Claim(req interface{}) (interface{}, interface{}) {
	...
	// 更新处理人
	err = s.ctx.DB.Ticket().UpdateAssignee(r.TenantId, r.TicketId, r.UserId, "")
	if err != nil {
		return nil, err
	}
	...
	if ticket.FirstResponseAt == 0 {
		ticket.FirstResponseAt = time.Now().Unix()
		ticket.ResponseTime = ticket.FirstResponseAt - ticket.CreatedAt
		err = s.ctx.DB.Ticket().Update(ticket)
		if err != nil {
			// 记录首次响应时间失败不影响主流程，仅记录错误
			// 可以考虑添加日志记录
		}
	}
...
func (s ticketService) Resolve(req interface{}) (interface{}, interface{}) {
	...
	// 更新工单状态为验证中
	ticket.Status = models.TicketStatusVerifying
	ticket.ResolvedAt = time.Now().Unix()
	...
	// 创建工作日志
	s.createWorkLog(r.TicketId, r.UserId, "resolve", "提交工单验证", "", "")
